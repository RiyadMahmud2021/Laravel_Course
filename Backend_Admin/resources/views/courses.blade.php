...
<div id="loaderDivCourse" class="container mt-5 pt-5">
   <div class="row">
      <div class="col-md-12 text-center pt-5">
         <img class="loading-icon m-5 animation_size" src="{{asset('images/loader.svg')}}">
      </div>
   </div>
</div>

<div id="WrongDivCourse" class="container d-none mt-5 pt-5">
   <div class="row">
   ...
   <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
         ...
         <div class="modal-body  text-center">
            ...
            <img id="courseEditLoader" class="animation_size m-5" src="{{asset('images/loader.svg')}}">
       ...
       axios.get('/getCourses')
           .then(function(response) {
               if (response.status == 200) {
                   ...
                   $('#courseDataTable').DataTable().destroy(); // firstly
                   $('#course_table').empty(); // secondly
   
                   var dataz = response.data;
                   $.each(dataz, function(i, item) {
                       $('<tr>').html(
                           "<td>" + dataz[i].course_name + "</td>" +
                           "<td>" + dataz[i].course_fee + "</td>" +
                           "<td>" + dataz[i].course_totalclass + "</td>" +
                           "<td>" + dataz[i].course_totalenroll + "</td>" +
                           "<td><a  class='courseEditBtn' data-id=" + dataz[i].id + "><i class='fas fa-edit'></i></a></td>" +
                           "<td><a  class='courseDeleteBtn'  data-id=" + dataz[i].id + " ><i class='fas fa-trash-alt'></i></a></td>"
                   ...
                   // end of the jquery table code block
       ...
       axios.post('/detailCourse', {
           ...
           .then(function(response) {
               if (response.status == 200) {
                   ...
                   var dataz = response.data;
                   $('#CourseNameUpdateId').val(dataz[0].course_name);
                   $('#CourseDesUpdateId').val(dataz[0].course_des);
                   $('#CourseFeeUpdateId').val(dataz[0].course_fee);
                   $('#CourseEnrollUpdateId').val(dataz[0].course_totalenroll);
                   $('#CourseClassUpdateId').val(dataz[0].course_totalclass);
                   $('#CourseLinkUpdateId').val(dataz[0].course_link);
                   $('#CourseImgUpdateId').val(dataz[0].course_img);
               } else {
                   $('#courseEditLoader').addClass('d-none');
